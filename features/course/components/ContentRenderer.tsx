    ...
    () => ({
      ...
      p: ({ children, ...props }: any) => {
        ...
        const hasCodeBlock = React.Children.toArray(children).some(
          ...
          (child: any) =>
            ...
            (child.type === 'pre' ||
              //eslint-disable-next-line
              (child.type === 'div' && (child.props as any)?.className?.includes('bg-gray-900'))),
        )
      ...
      code: ({ node, inline, className, children, ...props }: any) => {
        if (inline) {
          return (
            <code
              className="bg-beige-100 text-beige-800 px-2 py-1 rounded text-sm font-mono"
              {...props}
            >
              {children}
            </code>
      ...
      blockquote: ({ children, ...props }: any) => (
        <blockquote
          className="border-l-4 border-kuning-400 pl-4 py-2 mb-4 bg-beige-50 italic text-beige-700"
          {...props}
        >
          {children}
        </blockquote>
      ...
      th: ({ children, ...props }: any) => (
        <th
          className="border border-beige-300 px-4 py-2 text-left font-semibold text-beige-900"
          {...props}
        >
          {children}
        </th>
      ...
      strong: ({ children, ...props }: any) => (
        <strong
          className="font-bold text-beige-900 not-prose"
