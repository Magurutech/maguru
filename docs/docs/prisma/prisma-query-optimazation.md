...
Copy Markdown
Open
How to identify and optimize query performance with Prisma

This page covers identifying and optimizing query performance with Prisma ORM.

Query Insights
Query Insights is built into Prisma Postgres and shows you which queries are slow, how expensive they are, and what to fix. It works out of the box for raw SQL, but to see Prisma ORM operations (model name, action, query shape) you need one extra step.
...
Full table scans
Use Query Insights to identify which queries are affected and what to change.
...
Use the in filter
Set the relationLoadStrategy to "join"
