...
def main():
    try:
        ...
        if re.search(npm_pattern, command):
            ...
            blocked_command = command
            suggested_command = re.sub(r"\byarn\b", "bun", command)
        elif re.search(pnpm_pattern, command):
            blocked_command = command
            suggested_command = re.sub(r"\bpnpm\b", "bun", command)

        if blocked_command:
            # Log the usage attempt
            log_file = Path(__file__).parent.parent / "bun_enforcement.json"
            log_entry = {
            ...
            print("Error: Use 'bun/bunx' instead of 'npm/npx/yarn/pnpm'", file=sys.stderr)
